const text = {
    buttons : {
        mainMenu: {
            nearestAddress : "Найближчий пункт прийому вторсировини",
            howToPrepareWaste : "Як підготувати сміття до утилізації",
            addAddress: "Додати пункт прийому вторсировини",
            helpProject: "Допомогти проекту"
        }
    },
    info: {
        bot: "Привiт! Я бот, котрий допоможе тобi долучитися до вирiшення проблем переробки смiття!",
        addAddress: "Щоб додати пункт прийому вторинної сировини, заповни цю форму: bit.ly/2NPuvpT",
        contacts: "Ти можеш на написати нам пошту team.motiwaste@gmail.com або у телеграмі @yehorkuzmin",
    },
    messages: {
        meters: "метрiв",
        geolocationRequest: "Вiдправ менi, будь-ласка, свою геолокацiю",
        thanksForPhoto: "Дякую за твое фото",
        wasteType: "Який тип смiття хочеш здати?"
    }
}

module.exports = text